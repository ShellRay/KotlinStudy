...
class ViewPagerIndicator(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private var mRadiusSelected: Float = 0.toFloat()//选中半径，默认为mRadius
    ...
    private var mSpringPoint = arrayOf<Point>(Point(), Point(), Point(), Point(), Point(), Point())//arrayOfNulls<Point>(6)

    private var mControlPoint = arrayOf<Point>(Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point(), Point())
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (mIndicatorType) {
            ...
            -> {
                for (i in 0 until mNum) {//默认点 -(mNum - 1) * 0.5f * mDistance 第一个点
                    canvas.drawCircle((-(mNum - 1)).toFloat() * 0.5f * mDistance + i * mDistance, 0f, mRadius, paintStroke!!)
                }
                //选中
                canvas.drawCircle((-(mNum - 1)).toFloat() * 0.5f * mDistance + mOffset, 0f, mRadiusSelected, paintFill!!)
            }
            ...
            IndicatorType.PROGRESS//进度条
            -> {
                for (i in 0 until mNum) {//默认点 -(mNum - 1) * 0.5f * mDistance 第一个点
                    canvas.drawCircle((-(mNum - 1)).toFloat() * 0.5f * mDistance + i * mDistance, 0f, mRadius, paintStroke!!)
                }
                //选中
                val rightOpen = (-(mNum - 1)).toFloat() * 0.5f * mDistance + mOffset + mRadius
                val leftOpen = (-(mNum - 1)).toFloat() * 0.5f * mDistance - mRadius
                val topOpen = -mRadius
                val bottomOpen = mRadius
                val rectOpen = RectF(leftOpen, topOpen, rightOpen, bottomOpen)// 设置个新的长方形
     ...
     *
     *      不怎么好使的viewPager.adapter.count 数值通常很大
     * @return
     */
